...
1. Check out the branch intended for release (`develop` for major, `x.y` for minor). Make sure latest commits are pulled.
...
1. [Draft new release](https://github.com/ampproject/amp-wp/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor), with the new plugin version as the tag and release title.
    1. Attach the `amp.zip` build to the release.
    2. Add changelog entry to the release, link to compare view comparing previous release, and link to milestone.
...
1. Create built release tag (from the just-created `build` directory): `git fetch --tags && ./bin/tag-built.sh` (then add link from release notes)
1. For new major releases, create a release branch from the tag. Patch versions are made from the release branch.
1. Bump `Stable tag` in the `readme.txt`/`readme.md` in `develop`. Cherry-pick other changes as necessary.
1. Merge release tag into `master`.
1. For major release, publish release blog post, including link to GitHub release.
