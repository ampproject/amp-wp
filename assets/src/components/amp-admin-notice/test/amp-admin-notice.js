...
describe( 'AmpAdminNotice', () => {
	...
	it( 'renders a plain AMP admin notice', () => {
		...
		expect( container.querySelector( '.amp-admin-notice' ).textContent ).toBe( 'Content' );
	} );
	...
	it( 'renders a dismissible AMP admin notice', () => {
		...
		expect( container.querySelector( '.amp-admin-notice--dismissible' ) ).not.toBeNull();
		expect( container.querySelector( '.amp-admin-notice__dismiss' ) ).not.toBeNull();
	...
	it.each( [
		...
		act( () => {
			render(
				<AmpAdminNotice type={ type } />,
				container,
			);
		} );

		expect( container.querySelector( `.amp-admin-notice--${ type }` ) ).not.toBeNull();
	} );
} );
