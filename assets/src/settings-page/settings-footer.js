...
export function SettingsFooter() {
	...
	const isBusy =
		savingOptions || downloadingTheme || savingDeveloperToolsOption;
	const disabled =
		!hasChanges ||
		isBusy ||
		!themeSupport ||
		...
		<section className="amp-settings-nav">
			<div className="amp-settings-nav__inner">
				...
				{error && (
					<ErrorNotice
						errorMessage={
							error.message ||
							__(
								'An error occurred. You might be offline or logged out.',
								'amp'
							)
						}
					/>
				...
				{!error &&
					!hasChanges &&
					!downloadingTheme &&
					(didSaveOptions || didSaveDeveloperToolsOption) && (
						<AMPNotice
