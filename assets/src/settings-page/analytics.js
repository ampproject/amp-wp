...
function AnalyticsEntry({
	...
	useEffect(() => {
		...
		try {
			...
			if (
				...
				textAreaRef.current.setCustomValidity(
					__('A JSON object is required, e.g. {…}', 'amp')
				);
			} else {
				textAreaRef.current.setCustomValidity('');
			}
	...
	const isTextareaFocused = () => {
		...
		return (
			textAreaRef.current ===
			textAreaRef.current.ownerDocument.activeElement
		);
	};
	...
	const textareaControlValue =
		('' === config.trim() ||
			Object.values(vendorConfigs).find(
	...
	return (
		<PanelRow className="amp-analytics-entry">
			...
			<div
				...
				id={`amp-analytics-entry-${String(entryIndex)}`}
			>
				<div className="amp-analytics-entry__text-input">
					<label
						className="input-label"
						htmlFor={`amp-analytics-entry__text-input-${String(
							entryIndex
						)}`}
					>
						{__('Type:', 'amp')}
					...
					<input
						...
						id={`amp-analytics-entry__text-input-${String(
							entryIndex
						)}`}
					...
					/>
					<datalist
						id={`amp-analytics-vendors-${String(entryIndex)}`}
...
export function Analytics() {
	const { editedOptions, originalOptions, updateOptions } =
		useContext(Options);
				...
				<p>
					{createInterpolateElement(
						sprintf(
							...
							__(
								"Please see AMP project's <AnalyticsDocsLink>documentation</AnalyticsDocsLink> for %1$s as well as the <PluginAnalyticsDocsLink>plugin's analytics documentation</PluginAnalyticsDocsLink>. Each analytics configuration supplied below must take the form of a JSON object beginning with a %2$s and ending with a %3$s. Do not include any HTML tags like %4$s or %5$s. For the type field, supply one of the <VendorDocsLink>available analytics vendors</VendorDocsLink> or leave it blank for in-house analytics. For Google Analytics specifically, the type should be %6$s. For Google Tag Manager please consider using <SiteKitLink>Site Kit by Google</SiteKitLink> plugin.",
								'amp'
							),
						...
						),
						{
							...
							AnalyticsDocsLink: (
								<a
									href="https://amp.dev/documentation/components/amp-analytics/"
									target="_blank"
									rel="noreferrer"
								/>
							),
							PluginAnalyticsDocsLink: (
								<a
									href="https://amp-wp.org/documentation/getting-started/analytics/"
									target="_blank"
									rel="noreferrer"
								/>
							),
							VendorDocsLink: (
								<a
									href="https://amp.dev/documentation/guides-and-tutorials/optimize-and-measure/configure-analytics/analytics-vendors/"
									target="_blank"
									rel="noreferrer"
								/>
							),
