...
function ValidationErrorDetails({ errors }) {
	...
	return (
		<details>
			...
			<ul>
				{errors.map((error) => {
					const WrapperElement = !error.is_reviewed
						? 'strong'
						: Fragment;
					...
					return (
						...
						>
							<WrapperElement>
								...
								>
									<span>
										{createInterpolateElement(error.title, {
											code: <code />,
										})}
									</span>
								</a>
...
function PluginRow({ pluginKey, pluginDetails }) {
	...
	const isSuppressed =
		pluginKey in editedSuppressedPlugins &&
		...
		<div className="error-details">
			{isOriginallySuppressed ? (
				...
				<ValidationErrorDetails
					errors={pluginDetails.validation_errors}
				/>
			)}
		</div>
	);
	...
	return (
		<tr
			className={classnames({
				'has-border-color':
					isSuppressed || pluginDetails.validation_errors.length,
		...
		>
			<th className="column-status" scope="row">
				<SelectControl
					...
					onChange={() => {
						const newSuppressedPlugins = {
							...editedOptions.suppressed_plugins,
						};

						newSuppressedPlugins[pluginKey] = !isSuppressed;

						updateOptions({
							suppressed_plugins: newSuppressedPlugins,
						});
					}}
					value={isSuppressed}
			...
			<td className="column-plugin">
				<ConditionalDetails summary={<PluginName />}>
					{[
						pluginDetails.Author && (
							<p
								className="plugin-author-uri"
								key={`${pluginKey}-details-author`}
							>
						...
						pluginDetails.Description && (
							<div
								...
								dangerouslySetInnerHTML={{
									__html: autop(pluginDetails.Description),
								}}
							/>
						),
						pluginDetails.PluginURI && (
							<a
								...
								target="_blank"
								rel="noreferrer"
								key={`${pluginKey}-details-plugin-uri`}
							>
								{__('More details', 'amp')}
							</a>
						),
