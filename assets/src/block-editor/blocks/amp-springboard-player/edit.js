...
const BlockEdit = (props) => {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Springboard Player Settings', 'amp')}>
					<TextControl
						...
						value={dataSiteId}
						onChange={(value) =>
							setAttributes({ dataSiteId: value })
						}
					/>
					<TextControl
						...
						value={dataContentId}
						onChange={(value) =>
							setAttributes({ dataContentId: value })
						}
					/>
					<TextControl
					...
					<TextControl
						...
						value={dataDomain}
						onChange={(value) =>
							setAttributes({ dataDomain: value })
						}
					/>
					<SelectControl
					...
					<TextControl
						type="number"
						label={__(
							'Number of video is playlist (required)',
							'amp'
						)}
						value={dataItems}
						onChange={(value) =>
							setAttributes({ dataItems: value })
						}
					/>
					<LayoutControls
						{...props}
			...
			{url && (
				<MediaPlaceholder
					name={__('Springboard Player', 'amp')}
					url={url}
				/>
			)}
			{!url && (
