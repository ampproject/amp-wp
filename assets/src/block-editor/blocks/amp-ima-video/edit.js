		...
		<>
			<InspectorControls>
				<PanelBody title={__('IMA Video Settings', 'amp')}>
					<TextControl
						label={__(
							'HTTPS URL for your VAST ad document (required)',
							'amp'
						)}
						value={dataTag}
					...
					<TextControl
						label={__(
							'HTTPS URL of your video content (required)',
							'amp'
						)}
						value={dataSrc}
					...
					<TextControl
						...
						value={dataPoster}
						onChange={(value) =>
							setAttributes({ dataPoster: value })
						}
					/>
					<ToggleControl
			...
			{dataSet && (
				<MediaPlaceholder name={__('IMA Video', 'amp')} url={dataSrc} />
			)}
			{!dataSet && (
