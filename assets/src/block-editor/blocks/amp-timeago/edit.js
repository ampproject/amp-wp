...
import { DateTimePicker, PanelBody, TextControl } from '@wordpress/components';
...
const BlockEdit = (props) => {
	...
	if (dateTime) {
		if (
			cutoff &&
			parseInt(cutoff) <
				Math.abs(moment(dateTime).diff(moment(), 'seconds'))
		) {
		...
		setAttributes({
			dateTime: moment(moment(), moment.ISO_8601, true).format(),
		});
	}
