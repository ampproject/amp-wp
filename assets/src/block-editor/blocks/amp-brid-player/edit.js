...
const BlockEdit = (props) => {
	...
	if (
		dataPartner &&
		dataPlayer &&
		(dataVideo || dataPlaylist || dataOutstream)
	) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Brid Player Settings', 'amp')}>
					<TextControl
						...
						value={dataPartner}
						onChange={(value) =>
							setAttributes({ dataPartner: value })
						}
					/>
					<TextControl
						...
						value={dataPlayer}
						onChange={(value) =>
							setAttributes({ dataPlayer: value })
						}
					/>
					<TextControl
						label={__(
							'Video ID (one of video / playlist / outstream ID is required)',
							'amp'
						)}
						value={dataVideo}
						onChange={(value) =>
							setAttributes({ dataVideo: value })
						}
					/>
					<TextControl
						label={__(
							'Outstream unit ID (one of video / playlist / outstream ID is required)',
							'amp'
						)}
						value={dataOutstream}
						onChange={(value) =>
							setAttributes({ dataOutstream: value })
						}
					/>
					<TextControl
						label={__(
							'Playlist ID (one of video / playlist / outstream ID is required)',
							'amp'
						)}
						value={dataPlaylist}
						onChange={(value) =>
							setAttributes({ dataPlaylist: value })
						}
					/>
					<ToggleControl
			...
			{url && (
				<MediaPlaceholder name={__('Brid Player', 'amp')} url={url} />
			)}
			{!url && (
