		...
		<>
			<InspectorControls>
				<PanelBody title={__('JW Player Settings', 'amp')}>
					...
					<TextControl
						label={__(
							'Media ID (required if playlist ID not set)',
							'amp'
						)}
						value={dataMediaId}
						onChange={(value) =>
							setAttributes({ dataMediaId: value })
						}
					/>
					<TextControl
						label={__(
							'Playlist ID (required if media ID not set)',
							'amp'
						)}
						value={dataPlaylistId}
						onChange={(value) =>
							setAttributes({ dataPlaylistId: value })
						}
					/>
					<LayoutControls
						{...props}
			...
			{url && (
				<MediaPlaceholder name={__('JW Player', 'amp')} url={url} />
			)}
			{!url && (
