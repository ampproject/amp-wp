...
const BlockEdit = (props) => {
	...
	const { dataEmbedCode, dataPlayerId, dataPcode, dataPlayerVersion } =
		attributes;
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Ooyala Settings', 'amp')}>
					<TextControl
						...
						value={dataEmbedCode}
						onChange={(value) =>
							setAttributes({ dataEmbedCode: value })
						}
					/>
					<TextControl
						...
						value={dataPlayerId}
						onChange={(value) =>
							setAttributes({ dataPlayerId: value })
						}
					/>
					<TextControl
						label={__(
							'Provider code for the account (required)',
							'amp'
						)}
						value={dataPcode}
						onChange={(value) =>
							setAttributes({ dataPcode: value })
						}
					/>
					<SelectControl
						...
						onChange={(value) =>
							setAttributes({ dataPlayerVersion: value })
						}
					/>
					<LayoutControls
						{...props}
			...
			{url && (
				<MediaPlaceholder name={__('Ooyala Player', 'amp')} url={url} />
			)}
			{!url && (
