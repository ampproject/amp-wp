...
describe('After plugin activation', () => {
	...
	it('site scan is triggered automatically and displays no validation issues for AMP-compatible plugin', async () => {
		...
		await expect(page).toMatchElement('#amp-site-scan-notice p', {
		...
		await expect(page).toMatchElement(
			'#amp-site-scan-notice .amp-admin-notice--success'
		);
		await expect(page).not.toMatchElement('#amp-site-scan-notice summary');
		await expect(page).not.toMatchElement(
			'#amp-site-scan-notice .amp-site-scan-notice__cta'
		);
	});
	...
	it.each(['with Gutenberg active', 'without Gutenberg active'])(
		'site scan is triggered automatically and displays validation issues for AMP-incompatible plugin %s',
		async (title) => {
			...
			await expect(page).toMatchElement(
				'#amp-site-scan-notice .amp-admin-notice--warning'
			);
			await expect(page).toMatchElement('#amp-site-scan-notice summary', {
				text: /E2E Tests Demo Plugin/,
			});
			await expect(page).toMatchElement('#amp-site-scan-notice .button', {
