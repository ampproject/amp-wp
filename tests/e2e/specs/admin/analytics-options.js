...
describe( 'AMP analytics options', () => {
	...
	it( 'allows adding and deleting entries', async () => {
		...
		await expect( page ).toFill( '#amp-analytics-entry__new__-0 input[type="text"]', 'googleanalytics' );

		// Add second entry.
		await expect( page ).toClick( '#amp-analytics-add-entry' );
		await expect( '.amp-analytics-entry' ).countToBe( 2 );
		await expect( page ).toFill( '#amp-analytics-entry__new__-1 input[type="text"]', 'googleanalytics-2' );
