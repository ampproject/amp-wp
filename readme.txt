...
The plugin can be configured to follow one of three different template modes: Native, Transitional, or Reader. When configured to operate in Reader or Transitional modes, a given content type can have a canonical URL as well as a corresponding AMP URL. This does not mean that the site will be using a mobile theme, or that the AMP URL is used for redirecting users accessing the content from mobile devices. Instead, the AMP version will be served to mobile visitors when then encounter the content via platforms such as Twitter, Pinterest, Google Search, and others.
