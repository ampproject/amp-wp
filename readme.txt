...
4. **Support the serving of AMP pages** to make it easier for site owners to take advantage of mobile redirection, AMP-to-AMP linking, and generation of optimized AMP (e.g. AMP optimizer) by default.
...
**Transitional Mode**: In this mode there is also a single theme used, but there can be two versions of each page: AMP and non-AMP. The active theme is used for serving the AMP and non-AMP versions of a given URL. This mode is a good choice if the site uses a theme that is not fully AMP compatible, but the functional differences between the AMP and non-AMP are acceptable (due to graceful degradation). In this case users accessing the site from mobile devices can get the AMP version and get an optimized experience which also retains the look and feel of the non-AMP version. Check out our [showcase](https://amp-wp.org/showcases/?template_mode=transitional) of sites using Transitional Mode.
...
3. If you currently use older versions of the plugin in `Reader` mode, it is strongly encouraged to pick a Reader theme instead of using the legacy Reader templates. You may also want to migrate to `Transitional` or `Standard` mode; depending on your theme/plugins, some development work may be required.
