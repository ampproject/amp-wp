...
- Add support for the "page" post type, except when used as homepage or page for posts. A new `page.php` is introduced with template parts factored out and re-used from `single.php`. Note that AMP URLs will end in `?amp` instead of `/amp/`. See [#825](https://github.com/Automattic/amp-wp/pull/825). Props ThierryA, westonruter.
...
- Add ability to disable AMP on a per-post basis via toggle in publish metabox. See [#813](https://github.com/Automattic/amp-wp/pull/813). Props technosailor, ThierryA, westonruter.
