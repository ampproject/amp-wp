...
class AMP_Srcset_Sanitizer extends AMP_Base_Sanitizer {
	...
	private function sanitize_srcset_attr($attribute ) {
		...
		$element_attrs = array_map( static function ( $attribute ) {
		...
		$commas_count    = count( array_filter( $matches['comma'], static function ( $comma ) {
			return ',' === trim( $comma );
		} ) );
		...
		if ( $dimension_count !== count( $matches['url'] ) || ( $dimension_count - 1 ) !== $commas_count ) {
			$validation_error = [
				'code'               => AMP_Tag_And_Attribute_Sanitizer::INVALID_ATTR_VALUE,
				'element_attributes' => $element_attrs,
			];
		...
		if ( $dimension_count === count( array_flip( $matches['dimension'] ) ) ) {
			return;
		}
		...
		foreach ( $matches['dimension'] as $index => $dimension ) {
			...
			// Bail and raise a validation error if the dimension has another URL specified.
			...
			$image_candidates[ $dimension ] = $matches['url'];
		}
		...
		$attribute->textContent = $new_srcset;
	}
}
