	...
	public function sanitize() {
		...
		for ( $i = $num_nodes - 1; $i >= 0; $i-- ) {
			...
			if ( ! isset( $new_attributes['width'] ) || ! isset( $new_attributes['height'] ) ) {
				...
				// If only one dimension is set, don't ignore it; use that dimension to calculate the other.
				if ( ! $old_attributes['height'] && $old_attributes['width'] )  {
				...
				} else if ( ! $old_attributes['width'] && $old_attributes['height'] ) {
