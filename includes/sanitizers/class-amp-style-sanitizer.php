...
class AMP_Style_Sanitizer extends AMP_Base_Sanitizer {
	...
	private function finalize_styles() {
		...
		if ( $stylesheet_groups['custom']['included_count'] > 0 ) {
			...
			if ( $include_manifest_comment && ! empty( $included_sources ) && $included_original_size > 0 ) {
				...
				if ( self::has_required_php_css_parser() && ! self::disable_tree_shaking() ) {
					$comment .= sprintf(
	...
	private function finalize_stylesheet_group( $group, $group_config ) {
		...
		foreach ( $this->pending_stylesheets as $pending_stylesheet_index => &$pending_stylesheet ) {
			...
			foreach ( $pending_stylesheet['stylesheet'] as $stylesheet_part ) {
				...
				foreach ( $selectors_parsed as $selector => $parsed_selector ) {
					...
					if ( $should_include || self::disable_tree_shaking() ) {
						$selectors[] = $selector;
					}
				}
		...
		foreach ( $pending_stylesheet_indices as $i ) {
			...
			if ( $current_concatenated_size + $this->pending_stylesheets[ $i ]['size'] > $max_bytes && ! self::disable_tree_shaking() ) {
				$validation_error = [
					'code' => 'excessive_css',
					'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE,
				];
	 ...
	 */
	public static function disable_tree_shaking() {
		return isset( $_GET[ AMP_Theme_Support::AMP_FLAGS_QUERY_VAR ][ self::DISABLE_TREE_SHAKING_QUERY_VAR ] ) && AMP_Validation_Manager::has_cap(); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	}
}
