...
class AMP_Story_Legacy_Post_Type {
	...
	public static function register() {
		...
		// Select the single-legacy-amp-story.php template for AMP Stories.
	...
	public static function filter_template_include( $template ) {
		if ( is_singular( self::POST_TYPE_SLUG ) && ! is_embed() ) {
			$template = AMP__DIR__ . '/includes/templates/single-legacy-amp-story.php';
		}
		return $template;
	}
	...
	public static function render_block_with_google_fonts( $block_content, $block ) {
		...
		$font = AMP_Fonts::get_font( $block['attrs'][ $font_family_attribute ] );
		if ( ! $font ) {
			return $block_content;
		}
