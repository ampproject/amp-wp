...
def GeneratePHPAllowedTags(out_dir):
	"""Generates PHP class containing allowed tags and attributes for WordPress AMP plugin to consume.
	...
	# Write the php file.
	php_file = open(os.path.join(abs_path, 'class-amp-allowed-tags-generated.php'), 'w')
...
def GeneratePHPErrorCodes():
	logging.info('entering ...')

	versions = {}

	# Record the version of this specfile and the corresponding validator version.
	if validation_rules.HasField('spec_file_revision'):
		versions['spec_file_revision'] = validation_rules.spec_file_revision

	if validation_rules.HasField('min_validator_revision_required'):
		versions['min_validator_revision_required'] = validation_rules.min_validator_revision_required
	...
	out.append(' *')
	out.append(' * This is a list of error codes from the AMP specification. Note that ')
	out.append(' * tag names have been converted to lowercase.')
	out.append(' *')
	out.append(' * phpcs:ignoreFile')
	out.append(' */')
	out.append('class AMP_Error_Codes_Generated {')
	out.append('')
	...
	# add error codes
	error_codes = validator_pb2.ValidationError().Code
	error_formats = validation_rules.error_formats

	for error_format in error_formats:
		constant = error_codes.Name(error_format.code)
		value = UnicodeEscape(error_format.format).replace("'", "\\'")

		out.append(
			"\tconst {0} = '{1}';".format(constant, value)
		)
	out.append('')
	...
	# Write the php file.
	php_file = open(os.path.join(abs_path, 'class-amp-error-codes-generated.php'), 'w')
	php_file.write(output)
	php_file.close()
...
def ImportValidator():
	global validation_rules, validator_pb2
	...
	validation_rules = validator_pb2.ValidatorRules()
	text_format.Merge(open(specfile).read(), validation_rules)
...
def ParseRules(out_dir):
	...
	for (field_desc, field_val) in validation_rules.ListFields():
...
def Main( validator_directory, out_dir ):
	...
	ImportValidator()
	GeneratePHPAllowedTags(out_dir)
	GeneratePHPErrorCodes()
