...
do
	...
	until $(container bash -c "echo -n > /dev/tcp/mysql_$CONTAINER/3306" >/dev/null 2>&1); do
			echo -n '.'
			sleep 5
	done
	echo ''
	...
	# Install WordPress.
	echo -e $(status_message "Installing WordPress on $CONTAINER...")
	wp core install --title="AMP $CONTAINER" --admin_user=admin --admin_password=password --admin_email=test@test.com --skip-email --url=http://localhost:$HOST_PORT  --quiet
